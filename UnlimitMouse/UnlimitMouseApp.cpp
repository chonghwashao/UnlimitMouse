...
#include "stdafx.h"
#include "UnlimitMouseApp.h"
#include <stdio.h>

#define MAX_LOADSTRING 100
#define	WM_USER_SHELLICON WM_USER + 1
#define WM_USER_SET_CURSOR_POS (WM_USER + 2)
                     ...
                     int       nCmdShow)
{
	...
	while (GetMessage(&msg, NULL, 0, 0))
	{
		if (msg.hwnd == NULL)
		{
			if (msg.message == WM_USER_SET_CURSOR_POS)
			{
				SetCursorPos((int)msg.wParam, (int)msg.lParam);
			}
		}
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   //SetCursorPos(screenSize.cx / 2, screenSize.cy / 2);
   hMouseHook = SetWindowsHookEx( WH_MOUSE_LL,  MouseProc, hInstance, 0); 
   if (hMouseHook == NULL)
   {
	   WCHAR debugMsg[128];
	   wsprintf(debugMsg, _T("Install hook failed! Error code: %d\n"), GetLastError());
	   OutputDebugString((LPCWSTR)debugMsg);
   }
	...
	case WM_MOUSEMOVE:
		{
			...
			GetCursorPos(&pt);
			WCHAR debugMsg[128];
			wsprintf(debugMsg, TEXT("............: x = %d y = %d \n"), pt.x, pt.y );
			OutputDebugString((LPCWSTR)debugMsg);
			//BOOL flag = FALSE;
			//if (pt.x == screenSize.cx - 1)
			//{
			//	pt.x = 0;
			//	flag = TRUE;
			//}
			//if (pt.y == screenSize.cy - 1)
			//{
			//	pt.y = 0;
			//	flag = TRUE;
			//}
...
LRESULT CALLBACK MouseProc(int nCode, WPARAM wParam, LPARAM lParam) 
{
	...
	{		
		...
		WCHAR debugMsg[128];
		wsprintf(debugMsg, TEXT("Code: %d - Msg: %d - Mouse Postion: x = %d, y = %d\n"), nCode, wParam, pt.x, pt.y );
		OutputDebugString((LPCWSTR)debugMsg);
		// SetCursorPos(screenSize.cx / 2, screenSize.cy / 2);
		BOOL flag = FALSE;
		if(pt.x < 0 )
		...
		if(flag)
			PostThreadMessage(GetCurrentThreadId(), WM_USER_SET_CURSOR_POS, pt.x, pt.y);
	}
